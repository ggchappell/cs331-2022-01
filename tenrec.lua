...
-- REPL/Shell for Tenrec Programming Language
...
local tenrecstate  -- Tenrec variable values
...
local function printHelp()
    io.write("Type Tenrec code to execute it.\n")
    io.write("Commands (these may be abbreviated;")
    io.write(" for example, \":e\" for \":exit\")\n")
    io.write("  :exit          - Exit.\n")
    io.write("  :run FILENAME  - Execute Tenrec source file.\n")
    io.write("  :clear         - Clear Tenrec state.\n")
...
-- clearState
-- Clear Tenrec state: functions, simple variables, arrays.
...
-- runTenrec
-- Given a string, attempt to treat it as source code for a Tenrec
-- program, and execute it. I/O uses standard input & output.
--
-- Parameters:
--   program  - Tenrec source code
--   state    - Values of Tenrec variables as in interpit.interp.
...
function runTenrec(program, state, execmsg)
...
-- runFile
-- Given filename, attempt to read source for a Tenrec program from it,
...
function runFile(fname, printmsg)
    ...
    if not readable(fname) then
        errMsg("Tenrec source file not readable: '"..fname.."'")
        return
    end
    local source = ""
    ...
    good, done, tenrecstate = runTenrec(source, tenrecstate, execmsg)
    if not (good and done) then
        errMsg("Syntax error in Tenrec source file: '"..fname.."'")
    end
end
...
-- repl
-- Tenrec REPL. Prompt & get a line. If it begins with a colon (":")
-- then treat it as a REPL command. Otherwise, treat line as Tenrec
-- program, and attempt to execute it. If it looks like an incomplete
-- Tenrec program, then keep inputting, and continue to attempt to
-- execute. REPEAT.
function repl()
    ...
    while true do
        ...
        if line:sub(1,1) == ":" then    -- Command
            ...
            source = source .. line
            good, done, tenrecstate = runTenrec(source, tenrecstate)
            if (not good) and done then
...
-- Initialize Tenrec state
clearState()

-- Command-line argument? If so treat as Tenrec source filename, read
-- source, and execute.
if arg[1] ~= nil then
